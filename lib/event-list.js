...
EventList.prototype = {
  ...
  getActivityTaskSummary: function(name){
    ...
    result = _(this._events)
      ...
      .filter({eventType: 'ActivityTaskScheduled'})
      .filter(function(event){
        return event.eventAttributes.activityType.name === name;
      })
      .value();
    ...
    result = _(this._events)
      .chain()
      .filter(function(event){
        return scheduledEventIds.indexOf(event.eventAttributes.scheduledEventId) > -1
      })
      .value()

    //Create a summary
    var summary = {
      scheduled: scheduledEventIds.length,
      started: 0,
      canceled: 0,
      completed: 0,
      failed: 0
    ...
    result.forEach(function(event){
      switch(event.eventType){
        ...
        case 'ActivityTaskTimedOut':
          ...
          break;
        default:
          throw new Error('Unhandled eventAttributes.key "' + event.eventAttributes.key + '"');
      }
    });
