...
EventList.prototype = {
  ...
  mergeChildWorkflowResults: function(activityTypeName){
    return _(this._events)
      ...
      .filter(function(event){
        return (event && event.eventAttributes && event.eventAttributes.key === 'childWorkflowExecutionCompletedEventAttributes');
      })
      .map(function(event){
        return JSON.parse(event.eventAttributes.result || '{}');
      })
      .value();
  },
   ...
   * @param name (e.g., position-searchForWorker)
   */
  concatActivityResults: function(name){
    ...
    var scheduled = _(this._events)
      ...
      .filter(function(event){
        return (event && event.eventAttributes && event.eventAttributes.key === 'activityTaskScheduledEventAttributes');
      })
      .map(function(event){
        return {
  ...
  /**
   * Gets all signals received by the workflow
   * @param signalName Optional, filters signal events by signalName
   * @returns [{eventId, type, input}]
   */
  getSignalEvents: function(signalName){
    var me = this;
    return _(me._events)
      .chain()
      .filter({eventType: 'WorkflowExecutionSignaled'})
      .filter(function(event){
        return (event && event.eventAttributes && event.eventAttributes.key === 'workflowExecutionSignaledEventAttributes');
      })
      .filter(function(event){
        if(!signalName){
          return event;
          ...
          return event.eventAttributes.signalName === signalName;
        }
      })
      .map(function(event){
        return {
          eventId: event.eventId,
          type: event.eventAttributes.signalName,
          input: JSON.parse(event.eventAttributes.input || '{}')
        };
      })
      .value();
  },
  ...
  /**
   * Merge the results of all signals matching the signalName
   * @param signalName required
   * @returns {}
   */
  mergeSignalEvents: function(signalName){
    var me = this;
    return _(me.getSignalEvents(signalName))
      .chain()
      .reduce(_.merge)
      .value();
  },
  ...
  /**
   * Concatenate all of the values in a certain property
   * @param signalName required
   * @param propertyName required
   * @returns []
   */
  concatSignalEventsByProperty: function(signalName, propertyName){
    var me = this;
    return _(me.getSignalEvents(signalName))
      .chain()
      .map(function(event){
        return event.input[propertyName];
      })
      .value();
  },
   ...
   * @private
   */
  _getInitiatingEventIds: function(eventType, activityTypeName){
    var result = null;
    ...
    result = _(this._events)
      ...
      .filter(function(event){
        if(!activityTypeName) { //Limit to certain activity types?
          return event;
        }
        else{
          return event.eventAttributes.activityType.name === activityTypeName;
        }
      })
      .map(function(event){
        return event.eventId;
   ...
   * @param name
   */
  getActivityTaskSummary: function(activityTypeName){
    var result = null;

    //Get the eventIds to check for
    var initiatingEventIds = this._getInitiatingEventIds('ActivityTaskScheduled', activityTypeName);
