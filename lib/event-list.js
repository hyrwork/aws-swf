  ...
  _formatEventAttributes: function(events){
    return _(events)
      ...
      .map(function(event){
        ...
        if(eventAttributesKey.length !== 1){
          throw new Error('Invalid number of eventAttributesKeys found: ' + eventAttributesKey.length);
        }
        else {
    ...
    result.forEach(function(event){
      switch(event.eventType){
        ...
        case 'ChildWorkflowExecutionTimedOut':
          ...
          break;
        default:
          console.error('getChildWorkflowSummary unhandled eventAttributes.key "' + event.eventType + '"');
          //throw new Error('getChildWorkflowSummary unhandled eventAttributes.key "' + event.eventAttributes.key + '"');
      }
    });
    ...
    result.forEach(function(event){
      switch(event.eventType){
        ...
        case 'ActivityTaskTimedOut':
          summary.failed++;
          break;
        case 'ActivityTaskCompletedEventAttributes': // getActivityTaskSummary unhandl
          summary.completed++;
          break;
        default:
          console.error('getActivityTaskSummary unhandled eventAttributes.key "' + event.eventType + '"')
          //throw new Error('getActivityTaskSummary unhandled eventAttributes.key "' + event.eventAttributes.key + '"');
      }
    });
